...
class Transaksi_model extends CI_Model
{
    ...
    {
        $today = date('y-m-d');
        $query = $this->db->query("SELECT COUNT(id) as total_transaksi FROM booking WHERE tanggal_pesan LIKE '%$today%'");
        return $query->row();
    }
    public function getTotalTransaksiBulanIni()
    {
        $monthly = date('y-m');
        $query = $this->db->query("SELECT COUNT(id) as total_transaksi_bulan_ini FROM booking WHERE tanggal_pesan LIKE '%$monthly%'");
        return $query->row();
    }
